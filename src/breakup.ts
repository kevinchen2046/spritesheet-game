...
export class BreakUp {
    async exec(file, options) {
        ...
        if (options.format != "custom" && !FORMATS[options.format]) {
    ...
    /**
     * 从不包含原始序列图像宽高的数据中还原序列
     * @param bitmap 纹理
     * @param {{x:number,y:number,res:string}[]} frames  原始序列帧偏移数据
     * @param {{[res:string]:{x:number,y:number,w:number,h:number}}} source  位于图集的资源序列
